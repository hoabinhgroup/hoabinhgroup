...
try {
  ...
  if (!checkresultingsize($_FILES['files']['size'][0])) {
    ...
    $upload_handler->response['files'][0]->error =
  ...
  $uploadConfig = [
    ...
    'upload_url' =>
      $config['base_url'] . $config['upload_dir'] . $_POST['fldr'],
  ...
  if (!$config['ext_blacklist']) {
    ...
    $uploadConfig['accept_file_types'] =
      '/\.(?!' . implode('|', $config['ext_blacklist']) . '$)/i';

    if ($config['files_without_extension']) {
      $uploadConfig['accept_file_types'] =
        '/((\.(?!' .
        implode('|', $config['ext_blacklist']) .
        '$))|(^[^.]+$))/i';
    }
  }
  ...
  if ($ftp) {
    ...
    if (!is_dir($config['ftp_temp_folder'] . "thumbs")) {
      mkdir(
        $config['ftp_temp_folder'] . "thumbs",
        $config['folderPermission'],
        true
      );
    }
  ...
  // excute log
  $config['logs']['action'] = 'Thêm ảnh';
  $config['logs']['filename'] = json_encode([
    'path' => $_POST['fldr'],
