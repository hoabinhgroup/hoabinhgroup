...
if (!function_exists('response')) {
  ...
  {
    $responseClass = class_exists('Illuminate\Http\Response')
...
function isUploadDir($path, $config)
{
  ...
  if (
    realpath($path) === realpath($upload_dir) ||
    realpath($path) === realpath($thumbs_dir)
  ) {
    return true;
  }
  return false;
...
function logger($log_msg)
{
  ...
  $log_file_data = $log_filename . '/log_' . date('d-M-Y') . '.html';
...
function deleteFile($path, $path_thumb, $config)
{
  if ($config['delete_files']) {
    // $log_html = "---------------------------------";
    // $log_html .= "Thời gian: " . date('d/m/Y H:i:s', time()) . PHP_EOL;
    // $log_html .= "Hành động: Xóa ảnh" . PHP_EOL;
    // $log_html .= "File Xóa: " . $path . PHP_EOL;
    // $log_html .= "ID Người xóa: " . $_SESSION['info']['member']['id'] . PHP_EOL;
    // $log_html .=
    //   "Tên người xóa: " . $_SESSION['info']['member']['username'] . PHP_EOL;
    // logger($log_html);
        ...
        if (
          file_exists(
            $info['dirname'] .
              "/" .
              $path .
              $config['relative_image_creation_name_to_prepend'][$k] .
              $info['filename'] .
              $config['relative_image_creation_name_to_append'][$k] .
              "." .
              $info['extension']
          )
        ) {
  ...
  foreach ($files as $t) {
    if ($t != "." && $t != "..") {
      if (
        $count_hidden or
        !(
          in_array($t, $config['hidden_folders']) or
          in_array($t, $config['hidden_files'])
        )
      ) {
...
function checkresultingsize($sizeAdded)
{
  ...
  if ($config['MaxSizeTotal'] !== false && is_int($config['MaxSizeTotal'])) {
    list(
      ...
      $fileCurrentNum,
      $foldersCurrentCount,
    ) = folder_info($config['current_path'], false);
    // overall size over limit
    if (
      $config['MaxSizeTotal'] * 1024 * 1024 <
      $sizeCurrentFolder + $sizeAdded
    ) {
      return false;
    }
  }
...
function create_folder(
  ...
  if ($ftp) {
    ...
    if ($path_thumbs) {
      mkdir($path_thumbs, $permission, true) or
        die("$path_thumbs cannot be found");
    } // or even 01777 so you get the sticky bit set
    umask($oldumask);
    return true;
  }
...
function check_extension($extension, $config)
{
  ...
  if (
    (!$config['ext_blacklist'] && !in_array($extension, $config['ext'])) ||
  ...
  if (file_exists($img)) {
    ...
    if (ini_get('memory_limit') > 0) {
      ...
      if (strpos($mem, 'G') !== false) {
        $memory_limit = abs(
          intval(str_replace(['G'], '', $mem) * 1024 * 1024 * 1024)
        );
      }
      ...
      $image_memory_usage =
        $K64 + $image_width * $image_height * ($image_bits >> 3) * 2;
      $thumb_memory_usage =
        $K64 + $max_breedte * $max_hoogte * ($image_bits >> 3) * 2;
      $memory_needed = abs(
        intval($memory_usage + $image_memory_usage + $thumb_memory_usage)
      );
...
function new_thumbnails_creation(
  ...
  if ($config['fixed_image_creation']) {
    foreach ($config['fixed_path_from_filemanager'] as $k => $path) {
      ...
      $base_dir =
        $path . substr_replace($targetPath, '', 0, strlen($current_path));
...
function rcopy($source, $destination, $is_rec = false)
{
  if (is_dir($source)) {
    if ($is_rec === false) {
      ...
      $destination =
        rtrim($destination, '/') . DIRECTORY_SEPARATOR . $pinfo['basename'];
    }
...
function is_php($version = '5.0.0')
{
  ...
  if (!isset($phpVer[$version])) {
    $phpVer[$version] =
      version_compare(PHP_VERSION, $version) < 0 ? false : true;
  }
