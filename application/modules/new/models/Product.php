...
class New_Model_Product extends Louis_Db_Table_Abstract 
  ...
  $select = "SELECT p.*, m.name as menu_title FROM product as p inner join product_relationships as r inner join menu_items as m on p.id = r.object_id and m.id = r.category_id  WHERE m.link = '$ident' and p.lang= '$lang' and p.status = 1 order by id desc";
	...
	return $resultMaxID['maxcatid'];
	   ...
	   function getBeInterested(){
		    $select = $this->_db->select()
        				->from(array('r' => 'rating_summary'), array('id','votes'))
        				->joinInner(array('p' => $this->_name), 'p.id = r.id')
        				->joinInner(array('i' => 'productImage'), 
        							'p.id = i.productId', 
        							        array('full'));
        	$select->where('p.lang = ?', $this->_lang);
        	
        	$select->where('i.isDefault = ?', 'Yes');
        	$select->order('r.votes desc');			
        	$select->limit(8);	
			$result = $this->_db->fetchAll($select);
		     if(count($result) > 0) {
			 return $result;
