...
if (
  ...
  file_exists('lang/' . basename($_SESSION['RF']['language']) . '.php')
) {
...
function returnPaths($_path, $_name, $config)
{
  ...
  if ($ftp) {
    ...
    $path_thumb =
      $config['ftp_base_folder'] . $config['ftp_thumbs_dir'] . $_path;
  }
  if ($_name) {
...
if (
  ...
  !(isset($_GET['action']) && $_GET['action'] == 'delete_folder') &&
...
if (isset($_GET['action'])) {
  switch ($_GET['action']) {
    case 'delete_file':
      ...
      $config['logs']['action'] = 'Xóa ảnh';
      $config['logs']['filename'] = $path;
    ...
    case 'delete_files':
      ...
      $config['logs']['action'] = 'Xóa nhiều ảnh';
      $config['logs']['filename'] = json_encode($paths);
      break;
    case 'delete_folder':
      if ($config['delete_folders']) {
          ...
          if (is_dir($path)) {
            ...
            if ($config['fixed_image_creation']) {
              foreach ($config['fixed_path_from_filemanager'] as $k => $paths) {
                ...
                $base_dir =
                  $paths .
                  substr_replace($path, '', 0, strlen($config['current_path']));
                if (is_dir($base_dir)) {
        ...
        $config['logs']['action'] = 'Xóa thư mục';
        $config['logs']['filename'] = $path;
      }
      break;
    case 'create_folder':
      if ($config['create_folders']) {
        ...
        $res = create_folder(
          ...
          fix_path($path_thumb, $config),
          $ftp,
          $config
        );
        if (!$res) {
          response(
            trans('Rename_existing_folder') . AddErrorLocation()
          )->send();
        }
        $config['logs']['action'] = 'Tạo thư mục';
        $config['logs']['filename'] = $path;
      }
      break;
    case 'rename_folder':
      if ($config['rename_folders']) {
        ...
        if (!empty($name)) {
          ...
          $config['logs']['action'] = 'Đổi tên thư mục';
          $config['logs']['filename'] = $path . '=>' . $name;
    ...
    case 'create_file':
      if ($config['create_text_files'] === false) {
        response(
          sprintf(
            trans('File_Open_Edit_Not_Allowed'),
            strtolower(trans('Edit'))
          ) . AddErrorLocation()
        )->send();
        exit();
      }
      ...
      if (
        !isset($config['editable_text_file_exts']) ||
        !is_array($config['editable_text_file_exts'])
      ) {
      ...
      if (strpos($name, '.') === false) {
        response(
          trans('No_Extension') .
            ' ' .
            sprintf(
              trans('Valid_Extensions'),
              implode(', ', $config['editable_text_file_exts'])
            ) .
            AddErrorLocation()
        )->send();
        exit();
      ...
      if ($ftp) {
        ...
        if (!checkresultingsize(strlen($content))) {
          response(
            sprintf(trans('max_size_reached'), $config['MaxSizeTotal']) .
              AddErrorLocation()
          )->send();
          exit();
        }
      ...
      $config['logs']['action'] = 'Thêm file';
      $config['logs']['filename'] = $name;
      break;
      ...
      if ($config['rename_files']) {
        ...
        if (!empty($name)) {
          if (!rename_file($path, $name, $ftp, $config)) {
            response(
              trans('Rename_existing_file') . AddErrorLocation()
            )->send();
            exit();
          }
          ...
          $config['logs']['action'] = 'Đổi tên file';
          $config['logs']['filename'] = $path . '=>' . $name;
          ...
          if ($config['fixed_image_creation']) {
            ...
            foreach ($config['fixed_path_from_filemanager'] as $k => $paths) {
              ...
              if (
                file_exists(
                  $base_dir .
                    $config['fixed_image_creation_name_to_prepend'][$k] .
                    $info['filename'] .
      ...
      if ($ftp) {
        if ($action == 'copy') {
          ...
          if (url_exists($data['path_thumb'])) {
            @$ftp->rename(
              $data['path_thumb'],
              DIRECTORY_SEPARATOR . $path_thumb
            );
          }
        }
      } else {
        ...
        if (
          is_really_writable($path) === false ||
          is_really_writable($path_thumb) === false
        ) {
          response(
            trans('Dir_No_Write') .
              '<br/>' .
        ...
        if (
          is_function_callable($action == 'copy' ? 'copy' : 'rename') === false
        ) {
          response(
            sprintf(
              trans('Function_Disabled'),
              $action == 'copy' ? trans('Copy') : trans('Cut')
            ) . AddErrorLocation()
          )->send();
          exit();
        }
        if ($action == 'copy') {
          list($sizeFolderToCopy, $fileNum, $foldersCount) = folder_info(
            $path,
            false
          );
    ...
    case 'chmod':
      ...
      if ($chmod_perm === false) {
        response(
          sprintf(
            trans('File_Permission_Not_Allowed'),
            is_dir($path) ? trans('Folders') : trans('Files')
          ) . AddErrorLocation()
        )->send();
        exit();
      }
      ...
      if (!preg_match("/^[0-7]{3}$/", $mode)) {
        response(
          trans('File_Permission_Wrong_Mode') . AddErrorLocation()
        )->send();
        exit();
      }
      ...
      if (!$ftp && is_function_callable('chmod') === false) {
        response(
          sprintf(trans('Function_Disabled'), 'chmod') . AddErrorLocation()
        )->send();
        exit();
      }
