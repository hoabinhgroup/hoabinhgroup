 ...
 // $this->view->headScript()->offsetSetFile("1",$baseurl."/public/js/test2.js","text/javascrip");
    ...
    public function validemailAction()
    {
	    $this->_helper->viewRenderer->setNoRender(true);
	      ...
	      if($this->getRequest()->isXmlHttpRequest())
	      		{
		      $checkEmail = $this->_request->getPost("checkEmail");
		      
		      if($checkEmail){
			     $registration = new Model_Registration(); 
			  $email = $registration->get_one_where(array('email' => $checkEmail));  
			  if($email){
				  $success = false;
			  }else{
				  $success = true;
    ...
    public function validmobileAction()
    {
	    $this->_helper->viewRenderer->setNoRender(true);
	      $this->_helper->layout->disableLayout();
	      ...
	      if($this->getRequest()->isXmlHttpRequest())
	      		{
		      $checkMobile = $this->_request->getPost("checkMobile");
		      
		      if($checkMobile){
			     $registration = new Model_Registration(); 
			  $phone = $registration->get_one_where(array('mobile' => $checkMobile));  
			  if($phone){
				  $success = false;
			  }else{
				  $success = true;
