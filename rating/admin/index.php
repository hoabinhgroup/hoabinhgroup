...
$maxpage=ceil((isset($filter)?$itemstodisplay:$total["items"])/$show[$showIndex]);
...
<div class="container">
		...
		<div class="">
			...
			<ul class="nav nav-tabs">
			  <li class="active"><a href="#"><i class="icon-list"></i> List</a></li>
			  <li><a href="graph.php"><i class="icon-signal"></i> Graph</a></li>		
			</ul>
		...
		<div>
			<div class="pull-left">
				<h3 class="top-padding">List of items &nbsp;</h3>
			</div>
			...
			if(isset($itemstodisplay)) echo '<span class="label label-info pull-left">'.$itemstodisplay.' items found <a id="filteroffnow" href="index.php?o='.$order.'&amp;s='.$sortby.'&amp;n='.$showIndex.'" title="switch OFF filter"><i class="icon-remove icon-white"></i></a></span>';
			
			echo '<div class="pull-right">'; 
			$total["items"]=(isset($filter)?$itemstodisplay:$total["items"]);
			for($i=0;$i<count($show);$i++){
				if($total["items"]>$show[$i<1?0:$i-1]){
					if($i!=$showIndex) echo '<a href="'.$_SERVER['PHP_SELF'].'?o='.$order.'&amp;s='.$sortby.'&amp;n='.$i.'&amp;f='.$filter.'&amp;">'.$show[$i].'</a>';
					else echo '<b>'.$show[$i].'</b> per page';					
				}else break;
				if($total["items"]>$show[$i] && $i<count($show)-1) echo ' / ';
			}
			echo '&nbsp;</div>';
			...
			<table class="table table-striped">
				...
				<thead>
					<tr>
					  ...
					  <th><a href="<?php echo $_SERVER['PHP_SELF'].'?o='.($sortby==1?-$order:1).'&amp;s=1&amp;p=1&amp;n='.$showIndex.'&amp;f='.(isset($filter)?$filter:''); ?>&amp;" title="sort by title">Title / ID<i class="<?php echo ($sortby==1?'icon-chevron-'.($order<0?'up':'down').'':'icon-empty');?>"></i></a></th>
					  <th><a href="<?php echo $_SERVER['PHP_SELF'].'?o='.($sortby==2?-$order:1).'&amp;s=2&amp;p=1&amp;n='.$showIndex.'&amp;f='.(isset($filter)?$filter:''); ?>&" title="sort by rating">Rating<i class="<?php echo ($sortby==2?'icon-chevron-'.($order<0?'up':'down').'':'icon-empty');?>"></i></a></th>
					  <th><a href="<?php echo $_SERVER['PHP_SELF'].'?o='.($sortby==3?-$order:1).'&amp;s=3&amp;p=1&amp;n='.$showIndex.'&amp;f='.(isset($filter)?$filter:''); ?>&" title="sort by number of votes">Votes<i class="<?php echo ($sortby==3?'icon-chevron-'.($order<0?'up':'down').'':'icon-empty');?>"></i></a></th>
					  <th><span class="nolink">Top/Flop *</span></th>
					  <th><a href="<?php echo $_SERVER['PHP_SELF'].'?o='.($sortby==4?-$order:1).'&amp;s=4&amp;p=1&amp;n='.$showIndex.'&amp;f='.(isset($filter)?$filter:''); ?>&" title="sort by date">Open until<i class="<?php echo ($sortby==4?'icon-chevron-'.($order<0?'up':'down').'':'icon-empty');?>"></i></a></th>
					  <th><a href="<?php echo $_SERVER['PHP_SELF'].'?o='.($sortby==5?-$order:1).'&amp;s=5&amp;p=1&amp;n='.$showIndex.'&amp;f='.(isset($filter)?$filter:''); ?>&" title="time of repeat voting">&Delta;T *<i class="<?php echo ($sortby==5?'icon-chevron-'.($order<0?'up':'down').'':'icon-empty');?>"></i></a></th>
					<th><a href="<?php echo $_SERVER['PHP_SELF'].'?id='.(isset($id)?$id:'').'&amp;o='.($sortby==6?-$order:1).'&amp;s=6&amp;p=1&amp;n='.$showIndex.'&amp;f='.(isset($filter)?$filter:''); ?>&amp;" title="sort by date">Last vote<i class="<?php echo ($sortby==6?'icon-chevron-'.($order<0?'up':'down').'':'icon-empty');?>"></i></a></th>
				...
				<tbody>
					...
					$rows=getAllItems($cols[$sortby],$order,$page,$show[$showIndex],isset($filter)?$filter:'');
					...
					while($row = mysqli_fetch_array($rows)) {
						...
						if($row['numofid']>1){						
							echo '<a href="o='.$order.'&amp;s='.$sortby.'" class="subitemref" data-id="'.$row["n"].'" title="<b>show/hide '.($row['numofid']-1).' subitems</b>">'.($row["title"]).'<i class="icon-resize-full"></i></a></span>';
						}else{
					  ...
					  <td>
						<div class="progress progress-warning" title="<?php echo round($row["mean"]).'% ('.(round($row["mean"]/2)/10).' / 5 stars)';?>">
		...
		<?php
			if($maxpage>1){
				...
				echo getPagination($page, $maxpage, $_SERVER['PHP_SELF'].'?o='.$order.'&amp;s='.$sortby.'&amp;n='.$showIndex.'&amp;f='.(isset($filter)?$filter:''));
				echo '</div>';
			}
...
<?php
	...
	echo '<h1 class="pull-left offset50">There are no items in database</h1></div>';
}
